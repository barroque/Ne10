...
void seatest_assert_float_array_equal( float expected, float actual, unsigned int delta, unsigned int seatest_count, unsigned int loop_round, const char* function, unsigned int line )
{
    ...
    {
        sprintf(s, "Expected %e (0x%04X) but was %e (0x%04X) at position %d in loop round %d",
                expected, *(unsigned int*)&expected, actual, *(unsigned int*)&actual, seatest_count, loop_round);
...
int run_tests(seatest_void_void tests)
{
        ...
        seatest_header_printer(s, seatest_screen_width, ' ');
        sprintf(s,"in %lu micro-s",end - start);
        seatest_header_printer(s, seatest_screen_width, ' ');
        printf("\r\n");
